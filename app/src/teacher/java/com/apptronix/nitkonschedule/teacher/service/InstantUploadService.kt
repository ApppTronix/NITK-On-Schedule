                    ...
                    if (response.isSuccessful) {
                        if (response.code() == 200) {
                            ...
                            if(response.body()!!.result.equals("success")){
                                EventBus.getDefault().post(MarkStudents.MessageEvent("FaceRecognitionComplete",response.body()!!.data))

                            } else {
                                EventBus.getDefault().post(MainActivity.MessageEvent("Face recognition failed"))
                                Timber.e("face recognition response is failed")
